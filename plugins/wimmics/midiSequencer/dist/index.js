...
var WebAudioModule = class {
  ...
  async createGui() {
    ...
    if (!this._guiModuleUrl)
      return void 0;
    const { createElement: createElement2 } = await this._loadGui();
    return createElement2(this);
  }
  destroyGui() {
  }
};
...
// src/gui.ts
var MidiSequencerElement = class extends HTMLElement {
  module;
  root;
  $slider;
  $playing;
...
`;
    ...
    this.$loop = $container.querySelector("#loop");
    this.$loop.onchange = () => {
      module.audioNode.setParameterValues({ loop: { id: "loop", value: +this.$loop.checked, normalized: false } });
    };
    this.$now = $container.querySelector("#now");
    this.$total = $container.querySelector("#total");
    ...
    this.handleAnimationFrame = async () => {
      ...
      if (this.$playing.textContent !== temp)
        this.$playing.textContent = temp;
      if (this.$loop.checked !== !!loop.value)
        this.$loop.checked = !!loop.value;
      const { timeOffset, totalDuration } = module.audioNode;
      const date = new Date(timeOffset * 1e3);
      temp = date.toISOString().substring(11, 23);
      if (this.$now.textContent !== temp)
        this.$now.textContent = temp;
      temp = (timeOffset / totalDuration || 0).toString();
      if (this.$slider.value !== temp && this.root.activeElement !== this.$slider)
        this.$slider.value = temp;
      date.setTime(totalDuration * 1e3);
      temp = date.toISOString().substring(11, 23);
      if (this.$total.textContent !== temp)
        this.$total.textContent = temp;
...
var MidiParser_default = parseMidi;

// src/MidiSequencerProcessor.ts
var getMidiSequencerProcessor = (moduleId) => {
  const audioWorkletGlobalScope = globalThis;
  ...
  class MidiSequencerProcessor extends WamProcessor {
    ...
    sendFlush() {
      ...
      this.onMidi([176, 121, 0], currentTime);
      this.onMidi([176, 123, 0], currentTime);
    }
...
var MidiSequencerProcessor_default = getMidiSequencerProcessor;

// src/MidiSequencerNode.ts
var MidiSequencerNode = class extends WamNode {
  static async addModules(context, moduleId) {
    const { audioWorklet } = context;
    await super.addModules(context, moduleId);
    await addFunctionModule_default(audioWorklet, MidiSequencerProcessor_default, moduleId);
  }
  timeOffset;
  totalDuration;
  handleMessage;
  ...
  loadFile(file) {
    const data = MidiParser_default(file);
    this.totalDuration = data.duration;
...
var MidiSequencerNode_default = MidiSequencerNode;

// src/index.ts
var getBaseUrl = (relativeUrl) => {
  const baseUrl = relativeUrl.href.substring(0, relativeUrl.href.lastIndexOf("/"));
  return baseUrl;
};
var MidiSequencer = class extends WebAudioModule_default {
  _baseUrl = getBaseUrl(new URL(".", import.meta.url));
  _descriptorUrl = `${this._baseUrl}/descriptor.json`;
  async createAudioNode(initialState) {
    await MidiSequencerNode_default.addModules(this.audioContext, this.moduleId);
    const node = new MidiSequencerNode_default(this, {});
  ...
  async createGui() {
    return gui_default(this);
  }
};
var src_default = MidiSequencer;
export {
